...
class TestDecoding:
    ...
    def test_decode_to_rgba(self, heic_path):
        ...
        img = handle.decode(
            pylibheif.HeifColorspace.RGB, pylibheif.HeifChroma.InterleavedRGBA
        )
...
class TestEncoding:
    ...
    def test_encode_jpeg(self):
        ...
        descriptors = pylibheif.get_encoder_descriptors(
            pylibheif.HeifCompressionFormat.JPEG
        )
        if not descriptors:
...
class TestRoundTrip:
    ...
    def test_roundtrip(self, format_name, compression):
        ...
        try:
            ...
            # 允许有损压缩的误差
            mean_diff = np.abs(
                decoded_data.astype(float) - original_data.astype(float)
            ).mean()
