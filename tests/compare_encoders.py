...
def main():
    ...
    print(
        f"{'Encoder':<12} | {'Qual':<4} | {'Time (ms)':<10} | {'Size (KB)':<10} | {'PSNR (dB)':<10}"
    )
    print("-" * 60)
    ...
    for enc_name, fmt, id_filter in encoders:
        ...
        if id_filter:
            ...
            desc = [
                ...
                if d.id_name
                == ("x265" if fmt == pylibheif.HeifCompressionFormat.HEVC else "aom")
            ][0]
        ...
        for q in qualities:
            ...
            try:
                ms, size, psnr = benchmark_encoder(
                    enc_name, setup, heif_img, original_np
                )
                print(
                    f"{enc_name:<12} | {q:<4} | {ms:10.2f} | {size / 1024:10.2f} | {psnr:10.2f}"
                )
