...
def test_encoding_options_getters_setters():
    ...
    opts.preferred_chroma_downsampling_algorithm = pylibheif.HeifChromaDownsamplingAlgorithm.SharpYuv
    assert opts.preferred_chroma_downsampling_algorithm == pylibheif.HeifChromaDownsamplingAlgorithm.SharpYuv
    
    # Test preferred_chroma_upsampling_algorithm
    opts.preferred_chroma_upsampling_algorithm = pylibheif.HeifChromaUpsamplingAlgorithm.NearestNeighbor
    assert opts.preferred_chroma_upsampling_algorithm == pylibheif.HeifChromaUpsamplingAlgorithm.NearestNeighbor
...
def test_encode_with_options(heic_path):
    ...
    image = handle.decode(pylibheif.HeifColorspace.RGB, pylibheif.HeifChroma.InterleavedRGB)
...
async def test_async_encode_with_options(heic_path):
    ...
    handle = ctx.get_primary_image_handle()
    image = await handle.decode(pylibheif.HeifColorspace.RGB, pylibheif.HeifChroma.InterleavedRGB)
