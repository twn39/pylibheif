...
class HeifPlane {
public:
  HeifPlane(std::shared_ptr<HeifImage> img, heif_channel channel,
            bool writeable)
      : img(img), channel(channel), writeable(writeable) {}

  py::buffer_info get_buffer_info() {
    return img->get_buffer_info(channel, writeable);
  }
...
PYBIND11_MODULE(pylibheif, m) {
  ...
  py::enum_<heif_error_code>(m, "HeifErrorCode")
      ...
      .value("ColorProfileDoesNotExist",
  ...
  py::class_<HeifContext, std::shared_ptr<HeifContext>>(m, "HeifContext")
      ...
      .def("get_list_of_top_level_image_IDs",
  ...
  py::class_<HeifImageHandle, std::shared_ptr<HeifImageHandle>>(
      m, "HeifImageHandle")
  ...
  py::class_<HeifImage, std::shared_ptr<HeifImage>>(m, "HeifImage",
                                                    py::buffer_protocol())
      ...
      .def(
          "get_plane",
          [](std::shared_ptr<HeifImage> self, heif_channel channel,
