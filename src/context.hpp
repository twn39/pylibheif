...
class HeifContext {
   public:
    ...
    // Metadata writing
    void add_exif_metadata(std::shared_ptr<HeifImageHandle> handle, const py::bytes& data);
    void add_xmp_metadata(std::shared_ptr<HeifImageHandle> handle, const py::bytes& data);
    void add_generic_metadata(std::shared_ptr<HeifImageHandle> handle, const py::bytes& data,
