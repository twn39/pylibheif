...
int HeifImageHandle::get_width() const { return heif_image_handle_get_width(handle); }

int HeifImageHandle::get_height() const { return heif_image_handle_get_height(handle); }
...
std::vector<heif_item_id> HeifImageHandle::get_list_of_metadata_block_IDs(
    ...
    heif_image_handle_get_list_of_metadata_block_IDs(
        handle, type_filter.empty() ? nullptr : type_filter.c_str(), ids.data(), count);
    return ids;
}
...
py::buffer_info HeifImage::get_buffer_info(heif_channel channel, bool writeable) {
    ...
    if (writeable) {
        ...
        data = const_cast<uint8_t*>(heif_image_get_plane_readonly(image, channel, &stride));
    }
