...
public:
  ...
  std::shared_ptr<HeifImage> decode(heif_colorspace colorspace,
                                    heif_chroma chroma);

  // Metadata
  std::vector<heif_item_id>
  get_list_of_metadata_block_IDs(const std::string &type_filter = "");
...
public:
  ...
  HeifImage(int width, int height, heif_colorspace colorspace,
            heif_chroma chroma);
  ~HeifImage();
